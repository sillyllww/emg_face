...
# EMG数据的缓冲区
EMG_BUFFER_SIZE = 400  # 改为400帧
emg_data_buffer = []  # 将存储400帧EMG数据，每帧包含32通道
emg_buffer_lock = threading.Lock()  # 防止多线程同时访问缓冲区
buffer_frame_count = 0  # 计数器，记录当前缓冲区中的帧数

# 2000帧EMG数据缓冲区
EMG_BUFFER_SIZE_2000 = 2000
emg_data_buffer_2000 = []  # 将存储2000帧EMG数据
emg_buffer_2000_lock = threading.Lock()
buffer_2000_frame_count = 0
...
async def send_emg_buffer(buffer_data, prediction=None):
...
async def process_emg_queue():
    """处理EMG数据队列并维护100帧缓冲区，每积累100帧发送一次数据"""
    global emg_data_buffer, buffer_frame_count, emg_data_buffer_2000, buffer_2000_frame_count
    ...
    while True:
        ...
        try:
            while not emg_data_queue.empty():
                ...
                # 添加到2000帧缓冲区
                with emg_buffer_2000_lock:
                    # 如果缓冲区已满，移除最早的数据
                    if len(emg_data_buffer_2000) >= EMG_BUFFER_SIZE_2000:
                        emg_data_buffer_2000.pop(0)
                    emg_data_buffer_2000.append(new_data)
                    buffer_2000_frame_count += 1
        except Exception as e:
            pass
        ...
        if buffer_frame_count >= EMG_BUFFER_SIZE:
            try:
                ...
                # 将2000帧EMG数据缓冲区转换为numpy数组，调用process_and_predict，返回准确率
                if len(emg_data_buffer_2000) >= EMG_BUFFER_SIZE_2000:
                    # 转换为numpy数组，形状为(1, 2000, 16)
                    emg_array = np.array(emg_data_buffer_2000[-EMG_BUFFER_SIZE_2000:]).reshape(1, EMG_BUFFER_SIZE_2000, 16)
                    prediction_array = process_and_predict(emg_array)
                    # 将模型预测结果转换为前端期望的格式
                    if prediction_array is not None:
                        # 获取预测类别（最大概率的索引）和置信度（最大概率值）
                        predicted_class = int(np.argmax(prediction_array[0]))
                        confidence = float(np.max(prediction_array[0]))
                        prediction = {
                            "class": predicted_class,
                            "confidence": confidence
                        }
                    print(f"预测结果: 类别={prediction['class'] if prediction else None}, 置信度={prediction['confidence'] if prediction else None}")
                    
                if buffer_data:
                    # 发送数据，包含预测结果
                    await send_emg_buffer(buffer_data, prediction)
                    # 重置计数器
